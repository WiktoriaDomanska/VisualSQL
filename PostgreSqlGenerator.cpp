...
std::string PostgreSqlGenerator::generateSchemaSql(const Schema &schema, const std::vector<Link>& links, const std::map<std::string, std::vector<SqlTypeDefinition>>& types) {
    std::stringstream ss;

    for (const auto& table : schema.getTables()) {
        // Używamy podwójnych cudzysłowów
        ...
        for (size_t i = 0; i < columns.size(); ++i) {
            ss << "    \"" << columns[i].getName() << "\" ";

            // Jeśli kolumna ma Auto-Increment podmieniamy jej typ na SERIAL
            if (columns[i].getIsAutoIncrement()) {
                ss << "SERIAL";
            } else {
                ss << columns[i].getType();
                
                // Długość typu sprawdzamy tylko, jeśli nie użyliśmy SERIAL
                ...
                if (requiresLength) ss << "(" << columns[i].getLength() << ")";
            }
            ...
            if (!isLastColumn || !foreignKeys.empty()) ss << ",";
            ss << "\n";
        }

        for (size_t i = 0; i < foreignKeys.size(); ++i) {
            ss << foreignKeys[i];
            if (i < foreignKeys.size() - 1) ss << ",";
            ss << "\n";
        }
